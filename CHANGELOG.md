...
### Fixed
- Fix large upload indexing limits
- Fix release image pinning safeguards

### Changed
- Set compose file size limit default
- Clarify file size limit example
...
### Changed
- Restructured documentation for release readiness.
...
### Changed
- Clarify deployment guide roles
- Bypass rate limit for bulk indexing

## [2.14.1] - 2026-05-16

### Fixed
- Fix Windows drive bulk delete matching

### Changed
- Document stale Windows drive cleanup

## [2.14.0] - 2026-05-15

### Changed
- Implement API rate limiting and docs cleanup
...
### Fixed
- **Windows Installer Ref Pinning**: Release MSI now installs the **exact release tag** instead of `main`. The CI build injects `DEFAULT_REPO_REF = "vX.Y.Z"` and `$Branch = "vX.Y.Z"` into the frozen installer binary and `bootstrap_desktop_app.ps1` before PyInstaller runs, so users installing months later are guaranteed to receive the correct versioned code.
- **App Version Visible in Settings**: The Settings tab now displays the installed app version (e.g. `v2.11.1`) in the header so users can verify exactly what build is running.
...
### Added
- **Server-Side Edition Enforcement**: Backend now explicitly enforces `LICENSE_EDITION_RESTRICTED` (LIC_3006) for Organization features on Community/Loopback editions.
- **Organization UI Gating**: Desktop app admin tab detects `LIC_3006` and safely shows a gated placeholder view.
...
- **Permissions-tab Mapping**: Fixed permissions-tab mapping to accurately reflect role authorities.
- **Documents Tree-status Leakage**: Fixed Documents tree-status leakage in list mode.
- **Runtime Hardening**: General runtime hardening around offline and organization data loading.

## [2.8.1] - 2026-03-10

### Added
- **/me endpoint added**: New endpoint `/api/v1/me` successfully implemented to return identity profile.
...
### Fixed
- **License Validation**: Corrected a single-character typo (`t` → `u`) in the hardcoded fallback RS256 public key embedded in `license.py`. This resolves an issue where correctly signed newly issued licenses were rejected.

## [2.7.1] - 2026-03-07

### Added
- **Licensing/Commercial**: Updated pricing in COMMERCIAL.md to reflect the new $199/$349 Team blocks and added Phase D/E verification reference checks.

### Fixed
- **Settings UI**: License expiry text is no longer invisible on dark themes. `setStyleSheet("")` was resetting the text color to black, now explicitly sets `Theme.TEXT_PRIMARY`.
- **License Validation**: `license.py` now safely accepts the `organization` edition in the validation whitelist (previously rejected).
