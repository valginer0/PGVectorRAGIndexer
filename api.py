...
async def list_watched_folders(
    ...
    """List all watched folders with optional scope/executor filtering."""
...
async def add_watched_folder(request: Request):
    ...
    Body: { "folder_path": "...", "schedule_cron": "0 */6 * * *", "enabled": true,
            "client_id": "...", "execution_scope": "client", "executor_id": "...",
            "paused": false, "max_concurrency": 1 }
    """
    ...
    try:
        ...
        # Validate filesystem access for server-scope roots (#6b)
...
async def update_watched_folder(folder_id: str, request: Request):
    ...
    Body: { "enabled": true/false, "schedule_cron": "...", "paused": true/false, "max_concurrency": N }
    Note: execution_scope cannot be changed here — use /watched-folders/{id}/transition-scope.
    """
    from watched_folders import update_folder
    try:
        body = await request.json()

        # Reject execution_scope changes via generic update (#6b)
...
# ---------------------------------------------------------------------------
# Server-First Automation Endpoints (#6b)
# ---------------------------------------------------------------------------


@v1_router.post("/watched-folders/{folder_id}/transition-scope", tags=["Scheduling"], dependencies=[Depends(require_api_key)])
async def transition_folder_scope(folder_id: str, request: Request):
    ...
    Body: { "target_scope": "server" } or { "target_scope": "client", "executor_id": "..." }
    Preserves root_id. Returns 409 on conflict.
    """
    ...
    try:
        ...
        # Validate filesystem for server-scope transition
...
@v1_router.get("/scheduler/status", tags=["Scheduler Admin"], dependencies=[Depends(require_api_key)])
async def get_scheduler_status():
    """Get server scheduler status (lease, last poll, active scans)."""
    from server_scheduler import get_server_scheduler
    scheduler = get_server_scheduler()
    return scheduler.get_status()


@v1_router.get("/scheduler/roots/{root_id}/status", tags=["Scheduler Admin"], dependencies=[Depends(require_api_key)])
...
@v1_router.post("/scheduler/roots/{root_id}/pause", tags=["Scheduler Admin"], dependencies=[Depends(require_api_key)])
...
@v1_router.post("/scheduler/roots/{root_id}/resume", tags=["Scheduler Admin"], dependencies=[Depends(require_api_key)])
...
@v1_router.post("/scheduler/roots/{root_id}/scan-now", tags=["Scheduler Admin"], dependencies=[Depends(require_api_key)])
...
# ---------------------------------------------------------------------------
# Virtual Roots Endpoints (#9)
# ---------------------------------------------------------------------------


@v1_router.get("/virtual-roots", tags=["Path Mapping"], dependencies=[Depends(require_api_key)])
...
@v1_router.get("/virtual-roots/names", tags=["Path Mapping"], dependencies=[Depends(require_api_key)])
...
@v1_router.get("/virtual-roots/{name}/mappings", tags=["Path Mapping"], dependencies=[Depends(require_api_key)])
...
@v1_router.post("/virtual-roots", tags=["Path Mapping"], dependencies=[Depends(require_api_key)])
async def add_virtual_root(request: Request):
    """Add or update a virtual root mapping.

    Body: { "name": "...", "client_id": "...", "local_path": "..." }
    """
...
@v1_router.delete("/virtual-roots/{root_id}", tags=["Path Mapping"], dependencies=[Depends(require_api_key)])
...
@v1_router.post("/virtual-roots/resolve", tags=["Path Mapping"], dependencies=[Depends(require_api_key)])
async def resolve_virtual_path(request: Request):
    """Resolve a virtual path to a local path for a given client.

    Body: { "virtual_path": "FinanceDocs/reports/q1.pdf", "client_id": "..." }
    """
...
@v1_router.get("/documents/{document_id}/visibility", tags=["Document Visibility"], dependencies=[Depends(require_api_key)])
...
@v1_router.put("/documents/{document_id}/visibility", tags=["Document Visibility"], dependencies=[Depends(require_api_key)])
async def set_document_visibility_endpoint(document_id: str, request: Request):
    """Set visibility for a document.

    Body: {"visibility": "shared"|"private", "owner_id": "..."}
    Only the document owner or an admin can change visibility.
    """
    from document_visibility import (
        get_document_visibility, set_document_visibility,
        set_document_owner_and_visibility, set_document_owner,
    )
    try:
        body = await request.json()
...
@v1_router.post("/documents/{document_id}/transfer", tags=["Document Visibility"], dependencies=[Depends(require_admin)])
...
@v1_router.get("/users/{user_id}/documents", tags=["Document Visibility"], dependencies=[Depends(require_api_key)])
...
@v1_router.post("/documents/bulk-visibility", tags=["Document Visibility"], dependencies=[Depends(require_admin)])
async def bulk_set_visibility_endpoint(request: Request):
    """Set visibility for multiple documents at once (admin only).

    Body: {"document_ids": ["id1", "id2", ...], "visibility": "shared"|"private"}
    """
...
# ---------------------------------------------------------------------------
# SCIM 2.0 Provisioning (#16 Enterprise Foundations Phase 3)
# ---------------------------------------------------------------------------
...
@scim_router.get("/ServiceProviderConfig", tags=["SCIM"])
...
@scim_router.get("/Schemas", tags=["SCIM"])
...
@scim_router.get("/ResourceTypes", tags=["SCIM"])
...
@scim_router.get("/Users", tags=["SCIM"])
...
@scim_router.get("/Users/{user_id}", tags=["SCIM"])
...
@scim_router.post("/Users", tags=["SCIM"], status_code=201)
...
@scim_router.put("/Users/{user_id}", tags=["SCIM"])
...
@scim_router.patch("/Users/{user_id}", tags=["SCIM"])
...
@scim_router.delete("/Users/{user_id}", tags=["SCIM"], status_code=204)
async def scim_delete_user(user_id: str, request: Request):
    """Deactivate (soft-delete) a user via SCIM.

    Per SCIM convention, we deactivate rather than hard-delete.
    """
    _scim_auth(request)
