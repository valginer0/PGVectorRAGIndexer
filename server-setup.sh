...
if [ ! -f ".env" ]; then
    # Generate a password unique to this install. The previous value was the
    # literal string 'rag_password', published in this repo — and this script
    # sets up the networked server, where that mattered most.
...
EOF
    ...
    # Whichever branch ran, never leave a published placeholder in a real install.
    sed -i "s|^POSTGRES_PASSWORD=.*|POSTGRES_PASSWORD=${GENERATED_DB_PASSWORD}|" .env
    echo -e "  ✓ Created .env file (database password generated for this install)"
else
